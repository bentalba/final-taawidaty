<div align="center"><div align="center"><div align="center"><div align="center">



<img src="public/logos/TAAWIDATY.png" alt="TAAWIDATY Logo" width="180"/>  



# TAAWIDATY | تعويضاتي  <img src="public/logos/TAAWIDATY.png" alt="TAAWIDATY Logo" width="200"/>    <img src="public/logos/TAAWIDATY.png" alt="TAAWIDATY Logo" width="200"/>



### 🏥 Morocco's #1 Medication Reimbursement Calculator  



**Instant pricing • Accurate reimbursement • 8,111+ medications**  # TAAWIDATY | تعويضاتي  <img src="public/logos/TAAWIDATY.png" alt="TAAWIDATY Logo" width="200" style="border-radius: 20px;"/>  



[![Live Demo](https://img.shields.io/badge/🌐_Live-taawidaty.ma-0D7490?style=for-the-badge)](https://taawidaty.ma)  

[![Play Store](https://img.shields.io/badge/📱_Android-Play_Store-34A853?style=for-the-badge)](https://play.google.com/store/apps/details?id=com.taawidaty.app)

[![Version](https://img.shields.io/badge/Version-2.0.0-success?style=for-the-badge)](https://github.com/bentalba/final-taawidaty)  ### Morocco's #1 Medication Price & Reimbursement Calculator    # TAAWIDATY | تعويضاتي



---  



![React](https://img.shields.io/badge/React-18.3-61dafb?style=flat-square&logo=react)  <p>  # 🏥 TAAWIDATY | تعويضاتي  

![TypeScript](https://img.shields.io/badge/TypeScript-5.8-3178c6?style=flat-square&logo=typescript)

![Capacitor](https://img.shields.io/badge/Capacitor-7.0-119EFF?style=flat-square&logo=capacitor)    <strong>Instant pricing • Accurate reimbursement • 8,111+ medications</strong>

![Tailwind CSS](https://img.shields.io/badge/Tailwind-3.4-38bdf8?style=flat-square&logo=tailwind-css)

  </p>    ### 🏥 Calculateur de Remboursement des Médicaments - Maroc

</div>



---

  [![Live Demo](https://img.shields.io/badge/🌐_Live-taawidaty.ma-0077be?style=for-the-badge)](https://taawidaty.ma)  ### Morocco's #1 Medication Price & Reimbursement Calculator  

## 🌟 What is TAAWIDATY?

  [![Version](https://img.shields.io/badge/Version-2.0.0-success?style=for-the-badge)](https://github.com/bentalba/final-taawidaty)

**TAAWIDATY** (تعويضاتي - "My Reimbursements" in Arabic) is Morocco's most comprehensive medication price checker and reimbursement calculator. Instantly access official prices for **8,111+ medications** and calculate exact reimbursement from **CNOPS** or **CNSS** insurance.

  [![License](https://img.shields.io/badge/License-Proprietary-red?style=for-the-badge)](LICENSE)  ### حاسبة أسعار الأدوية والتعويضات في المغرب  **Designed & Developed by [BENTALBA ZAKARIA](https://github.com/salma1-create)**

### 🎯 Perfect For

  

| User | Benefit |

|------|---------|  <br/>    

| 💊 **Patients** | Check medication prices before visiting the pharmacy |

| 🏥 **Healthcare Professionals** | Quick reference for medication costs |

| 📊 **Insurance Holders** | Calculate exact CNOPS/CNSS reimbursement |

| 👨‍👩‍👧‍👦 **Families** | Budget healthcare expenses accurately |  ![React](https://img.shields.io/badge/React-18.3.1-61dafb?style=flat-square&logo=react&logoColor=white)  <p align="center">  [![React](https://img.shields.io/badge/React-18.3.1-61dafb?style=flat&logo=react)](https://reactjs.org/)



---  ![TypeScript](https://img.shields.io/badge/TypeScript-5.8.3-3178c6?style=flat-square&logo=typescript&logoColor=white)



## ✨ Features  ![Vite](https://img.shields.io/badge/Vite-5.4.19-646cff?style=flat-square&logo=vite&logoColor=white)    <strong>Instant pricing • Accurate reimbursement • 8,111+ medications</strong>  [![TypeScript](https://img.shields.io/badge/TypeScript-5.8.3-3178c6?style=flat&logo=typescript)](https://www.typescriptlang.org/)



### Core Functionality  ![Tailwind CSS](https://img.shields.io/badge/Tailwind-3.4.17-38bdf8?style=flat-square&logo=tailwind-css&logoColor=white)

- 🔍 **Smart Search** - Find medications by name or active ingredient (DCI)

- 💰 **Price Display** - Official PPV prices in bold, easy to read  </p>  [![Vite](https://img.shields.io/badge/Vite-5.4.19-646cff?style=flat&logo=vite)](https://vitejs.dev/)

- 🧮 **Reimbursement Calculator** - Instant CNOPS & CNSS calculations

- 📱 **Native Android App** - Available on Google Play Store  <br/>



### User Experience  [![Tailwind CSS](https://img.shields.io/badge/Tailwind-3.4.17-38bdf8?style=flat&logo=tailwind-css)](https://tailwindcss.com/)

- 🌙 **Dark Mode** - Easy on the eyes, day or night

- 🌐 **Bilingual** - Full Arabic (RTL) and French support  ![Lighthouse Performance](https://img.shields.io/badge/Performance-95+-success?style=flat-square&logo=lighthouse)

- ⚡ **Offline Mode** - Works without internet (cached data)

- 📊 **Calculation History** - Track past calculations  ![Lighthouse Accessibility](https://img.shields.io/badge/Accessibility-100-success?style=flat-square&logo=lighthouse)  [![Live Demo](https://img.shields.io/badge/🌐_Live_Demo-taawidaty.ma-0077be?style=for-the-badge)](https://taawidaty.ma)  [![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)



### Technical  ![Lighthouse SEO](https://img.shields.io/badge/SEO-100-success?style=flat-square&logo=lighthouse)

- 🚀 **Lightning Fast** - Optimized for Moroccan networks

- 📱 **Mobile First** - Touch-optimized, smooth scrolling  ![Core Web Vitals](https://img.shields.io/badge/Core_Web_Vitals-Passing-success?style=flat-square)  [![Version](https://img.shields.io/badge/Version-2.0.0-success?style=for-the-badge)](https://github.com/bentalba/final-taawidaty)

- ♿ **Accessible** - WCAG 2.1 compliant

- 🔒 **Privacy First** - No account required, local storage only



---</div>  [![License](https://img.shields.io/badge/License-Proprietary-red?style=for-the-badge)](LICENSE)  [🌐 Demo](https://dawa-calcul-plus.vercel.app) • [📖 Documentation](#-features) • [🤝 Contributing](#-contributing)



## 🛠️ Tech Stack



| Category | Technology |---  

|----------|------------|

| **Frontend** | React 18, TypeScript, Tailwind CSS |

| **Build** | Vite 5, Bun |

| **Mobile** | Capacitor 7 (Android) |## 🌟 What is TAAWIDATY?  <br/></div>

| **Animation** | Framer Motion |

| **Icons** | Lucide React |



---**TAAWIDATY** (تعويضاتي - meaning "My Reimbursements" in Arabic) is Morocco's most comprehensive medication price checker and reimbursement calculator. Get instant access to official prices for **8,111+ medications** and calculate your exact reimbursement from **CNOPS** or **CNSS** insurance.



## 🚀 Quick Start



### Prerequisites### 🎯 Perfect For  [![React](https://img.shields.io/badge/React-18.3.1-61dafb?style=flat-square&logo=react&logoColor=white)](https://reactjs.org/)---

- [Bun](https://bun.sh/) (recommended) or Node.js 18+

- Android Studio (for mobile development)



### Installation- 💊 **Patients** - Check medication prices before visiting the pharmacy  [![TypeScript](https://img.shields.io/badge/TypeScript-5.8.3-3178c6?style=flat-square&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)



```bash- 🏥 **Healthcare Professionals** - Quick reference for medication costs

# Clone the repository

git clone https://github.com/bentalba/final-taawidaty.git- 📊 **Insurance Holders** - Calculate exact reimbursement amounts (CNOPS/CNSS)  [![Vite](https://img.shields.io/badge/Vite-5.4.19-646cff?style=flat-square&logo=vite&logoColor=white)](https://vitejs.dev/)## 📋 Table of Contents

cd final-taawidaty

- 👨‍👩‍👧‍👦 **Families** - Budget healthcare expenses accurately

# Install dependencies

bun install- 🇲🇦 **Moroccan Citizens** - Access official government pricing data  [![Tailwind CSS](https://img.shields.io/badge/Tailwind-3.4.17-38bdf8?style=flat-square&logo=tailwind-css&logoColor=white)](https://tailwindcss.com/)



# Start development server

bun run dev

```---  [![Framer Motion](https://img.shields.io/badge/Framer_Motion-11.18.0-ff69b4?style=flat-square&logo=framer&logoColor=white)](https://www.framer.com/motion/)- [Overview](#-overview)



### Build for Production



```bash## ✨ Key Features- [Features](#-features)

# Web build

bun run build



# Android build<table>  <br/>- [Tech Stack](#-tech-stack)

bun run build

bunx cap sync android  <tr>

cd android && ./gradlew assembleDebug

```    <td width="50%">- [Getting Started](#-getting-started)



---      



## 📁 Project Structure### 🔍 **Smart Search**  [![Lighthouse Performance](https://img.shields.io/badge/Performance-95+-success?style=flat-square&logo=lighthouse)](https://developers.google.com/web/tools/lighthouse)- [Project Structure](#-project-structure)



```- Search 8,111+ medications instantly

taawidaty/

├── src/- Autocomplete with fuzzy matching  [![Lighthouse Accessibility](https://img.shields.io/badge/Accessibility-100-success?style=flat-square&logo=lighthouse)](https://developers.google.com/web/tools/lighthouse)- [Usage](#-usage)

│   ├── components/     # Reusable UI components

│   ├── pages/          # Route pages- Search by brand name, DCI, or barcode

│   ├── hooks/          # Custom React hooks

│   ├── data/           # Medication JSON databases- Multi-language support (FR/AR)  [![Lighthouse SEO](https://img.shields.io/badge/SEO-100-success?style=flat-square&logo=lighthouse)](https://developers.google.com/web/tools/lighthouse)- [Bilingual Support](#-bilingual-support)

│   ├── utils/          # Helper functions

│   └── lib/            # Translations, utilities

├── public/             # Static assets

├── android/            # Capacitor Android project### 💰 **Accurate Pricing**  [![Core Web Vitals](https://img.shields.io/badge/Core_Web_Vitals-Passing-success?style=flat-square)](https://web.dev/vitals/)- [FAQ System](#-faq-system)

└── docs/               # Documentation

```- Official pharmacy prices (PPV)



---- Hospital prices (PH)- [Contributing](#-contributing)



## 📊 Data Sources- Public sector pricing



| Insurance | Medications | Source |- Real-time database</div>- [License](#-license)

|-----------|-------------|--------|

| **CNOPS** | 4,200+ | Official CNOPS database |

| **CNSS** | 3,900+ | Official CNSS database |

### 📊 **Reimbursement Calculator**

*Data is updated periodically from official government sources.*

- CNOPS rates (70%, 90%, 100%)

---

- CNSS reimbursement------

## 🤝 Contributing

- Instant calculation

Contributions are welcome! Please feel free to submit a Pull Request.

- Detailed breakdown

1. Fork the repository

2. Create your feature branch (`git checkout -b feature/amazing-feature`)

3. Commit your changes (`git commit -m 'Add amazing feature'`)

4. Push to the branch (`git push origin feature/amazing-feature`)    </td>## 🌟 What is TAAWIDATY?## 🎯 Overview

5. Open a Pull Request

    <td width="50%">

---



## 📄 License

### 🌍 **Bilingual Interface**

This project is proprietary software. The code structure is available for reference, but the **TAAWIDATY** brand name, logo, and medication database are protected.

- Full French & Arabic**TAAWIDATY** is Morocco's most comprehensive medication price checker and reimbursement calculator. Get instant access to official prices for **8,111+ medications** and calculate your exact reimbursement from **CNOPS** or **CNSS** insurance - all in under 2 seconds.**TAAWIDATY** (تعويضاتي) is a comprehensive web application designed to help Moroccan citizens calculate medication reimbursement rates for both **CNOPS** and **CNSS** insurance systems. With an extensive database of **5,709+ medications** and full bilingual support (French/Arabic), it provides instant, accurate reimbursement calculations.

See [LICENSE](LICENSE) for details.

- RTL support for Arabic

---

- Seamless switching

## 👨‍💻 Author

- Culturally adapted

**BENTALBA ZAKARIA**

### 🎯 Perfect For:### 🎬 Why TAAWIDATY?

- GitHub: [@bentalba](https://github.com/bentalba)

- Website: [taawidaty.ma](https://taawidaty.ma)### 📱 **Mobile Optimized**



---- Responsive design



<div align="center">- PWA ready



**Made with ❤️ in Morocco 🇲🇦**- Works offline- 💊 **Patients**: Check medication prices before visiting the pharmacy- **Transparency**: Know your reimbursement rates before visiting the pharmacy



</div>- Fast loading (1.5s)


- 🏥 **Healthcare Professionals**: Quick reference for medication costs- **Comprehensive**: Access to complete medication databases for both insurance systems

### 🎨 **Modern UI/UX**

- Gradient animations- 📊 **Insurance Holders**: Calculate exact reimbursement amounts (CNOPS/CNSS)- **Accessible**: Full bilingual support with RTL (Right-to-Left) for Arabic

- Dark mode

- Smooth transitions- 👨‍👩‍👧‍👦 **Families**: Budget healthcare expenses accurately- **User-Friendly**: Modern, responsive interface optimized for all devices

- Glassmorphism

- 🇲🇦 **Moroccan Citizens**: Access official government pricing data- **Educational**: Detailed FAQ sections to understand insurance processes

    </td>

  </tr>

</table>

------

---



## 🚀 Performance

## ✨ Key Features## ✨ Features

<div align="center">



### Lighthouse Scores (Mobile)

<table>### 🔍 Medication Search

| Metric | Score | Status |

|--------|-------|--------|  <tr>- **Instant Search**: Real-time filtering across 5,709+ medications

| ⚡ Performance | 95+ | 🟢 Excellent |

| ♿ Accessibility | 100 | 🟢 Perfect |    <td width="50%">- **Dual Database**: Separate databases for CNOPS and CNSS

| 🎯 Best Practices | 100 | 🟢 Perfect |

| 🔍 SEO | 100 | 🟢 Perfect |      - **Smart Filtering**: Search by medication name with instant results



### Core Web Vitals### 🔍 **Smart Search**- **Detailed Information**: View medication codes, prices, and reimbursement rates



| Metric | Value | Status |- Search 8,111+ medications instantly

|--------|-------|--------|

| LCP (Largest Contentful Paint) | 1.5s | 🟢 Good |- Autocomplete with fuzzy matching### 💰 Reimbursement Calculator

| FID (First Input Delay) | 0ms | 🟢 Good |

| CLS (Cumulative Layout Shift) | 0.006 | 🟢 Good |- Search by brand name, generic name (DCI), or barcode- **Automatic Calculation**: Instant computation of reimbursement amounts



</div>- Multi-language support (French/Arabic)- **Clear Breakdown**: 



---  - Public Price (Prix Public)



## 🛠️ Tech Stack### 💰 **Accurate Pricing**  - Reimbursement Rate (Taux de Remboursement)



### Core- Official pharmacy prices (PPV)  - Reimbursed Amount (Montant Remboursé)

- **React 18.3** - UI library

- **TypeScript 5.8** - Type safety- Hospital prices (PH)  - Patient Responsibility (Reste à Charge)

- **Vite 5.4** - Build tool

- **Tailwind CSS 3.4** - Styling- Public sector pricing



### UI/UX- Real-time price database### 🌍 Bilingual Interface

- **Framer Motion** - Animations

- **Radix UI** - Accessible components- **Full Language Support**: Complete French and Arabic translations

- **Lucide React** - Icons

### 📊 **Reimbursement Calculator**- **RTL Support**: Proper Right-to-Left rendering for Arabic

### State & Forms

- **React Hook Form** - Forms- CNOPS reimbursement (70%, 90%, 100%)- **Persistent Language**: Language preference saved across sessions

- **Zod** - Validation

- **React Router** - Routing- CNSS reimbursement rates- **Seamless Switching**: Toggle between languages instantly



### Deployment- Instant calculation

- **Cloudflare Pages** - CDN hosting

- **GitHub Actions** - CI/CD- Detailed breakdown### ❓ Comprehensive FAQ



---- **CNOPS FAQ**: 15 detailed questions covering all aspects of CNOPS insurance



## 📊 Database    </td>- **CNSS FAQ**: 15 detailed questions covering all aspects of CNSS insurance



- **Total Medications**: 8,111    <td width="50%">- **Bilingual Content**: All FAQs available in French and Arabic

- **CNOPS Coverage**: Complete

- **CNSS Coverage**: Complete- **Easy Navigation**: Quick access from landing page with multiple entry points

- **Data Source**: Official Moroccan government databases

- **Update Frequency**: Quarterly### 🌍 **Bilingual Interface**



### Data Fields- Full French & Arabic support### 📱 Responsive Design

- Brand & generic names

- DCI (International Nonproprietary Name)- RTL (Right-to-Left) for Arabic- **Mobile-First**: Optimized for smartphones and tablets

- PPV (Pharmacy Public Price)

- PH (Hospital Price)- Culturally adapted UI/UX- **Desktop Enhanced**: Rich experience on larger screens

- Reimbursement rates

- Barcode / EAN- Seamless language switching- **Accessibility**: Built with a11y best practices using Radix UI



---



## 🚀 Getting Started### 📱 **Mobile Optimized**---



### Prerequisites- Responsive design (mobile-first)

- Node.js 18+ or Bun

- Git- Progressive Web App (PWA) ready## 🛠 Tech Stack



### Installation- Works offline (service worker)



```bash- Fast loading (1.5s FCP)### Frontend Framework

# Clone repository

git clone https://github.com/bentalba/final-taawidaty.git- **React 18.3.1**: Modern UI with hooks and context

cd final-taawidaty

### 🎨 **Modern UI/UX**- **TypeScript 5.8.3**: Type-safe development

# Install dependencies

npm install- Beautiful gradient animations- **Vite 5.4.19**: Lightning-fast build tool and dev server

# or

bun install- Dark mode support



# Start dev server- Smooth transitions (Framer Motion)### UI & Styling

npm run dev

# or- Glassmorphism design- **Tailwind CSS 3.4.17**: Utility-first styling

bun dev

```- **shadcn/ui**: High-quality accessible components



Visit `http://localhost:5173` 🎉    </td>- **Radix UI**: Primitive components for accessibility



### Build for Production  </tr>- **Lucide React**: Beautiful icon system



```bash</table>

# Build

npm run build### Routing & State



# Preview---- **React Router DOM 6.30.1**: Client-side routing

npm run preview

```- **Context API**: Global state management for language



---## 🚀 Performance- **LocalStorage**: Persistent user preferences



## 📁 Project Structure



```<div align="center">### Development Tools

taawidaty/

├── src/- **ESLint**: Code quality and consistency

│   ├── components/          # React components

│   │   ├── ui/             # Radix UI components### Lighthouse Scores (Mobile)- **PostCSS**: CSS transformations

│   │   ├── SearchInput.tsx # Smart search

│   │   └── ResultCard.tsx  # Results display- **Python 3.9+**: Data processing scripts

│   ├── data/               # Medication databases

│   │   ├── medications-cnops.json| Metric | Score | Status |

│   │   └── medications-cnss.json

│   ├── hooks/              # Custom hooks|--------|-------|--------|---

│   ├── lib/                # Utilities

│   ├── pages/              # Page components| ⚡ Performance | 95+ | 🟢 Excellent |

│   └── App.tsx             # Main app

├── public/                 # Static assets| ♿ Accessibility | 100 | 🟢 Perfect |## 🚀 Getting Started

│   ├── logos/

│   └── sitemap*.xml| 🎯 Best Practices | 100 | 🟢 Perfect |

├── docs/                   # Documentation

└── package.json| 🔍 SEO | 100 | 🟢 Perfect |### Prerequisites

```



---

### Core Web Vitals- **Node.js** (v18 or higher) - [Install with nvm](https://github.com/nvm-sh/nvm#installing-and-updating)

## 🌍 Bilingual Support

- **npm** or **yarn** or **bun**

| Feature | French | Arabic |

|---------|--------|--------|| Metric | Value | Status |

| **Interface** | ✅ Full | ✅ Full |

| **Search** | ✅ | ✅ ||--------|-------|--------|### Installation

| **Text Direction** | LTR | RTL ✅ |

| **Font** | Latin | Arabic || LCP (Largest Contentful Paint) | 1.5s | 🟢 Good |

| **SEO** | ✅ | ✅ |

| FID (First Input Delay) | 0ms | 🟢 Good |```bash

Switch languages instantly with the toggle button 🇫🇷/🇲🇦

| CLS (Cumulative Layout Shift) | 0.006 | 🟢 Good |# Clone the repository

---

git clone https://github.com/salma1-create/dawa-calcul-plus.git

## 🔍 SEO & Performance

**Speed Index**: 2.5s • **Time to Interactive**: 2.8s • **Total Size**: 87KB

### SEO Features

- ✅ 100/100 Lighthouse SEO# Navigate to project directory

- ✅ 8,124 URLs indexed

- ✅ Proper H1 tags</div>cd dawa-calcul-plus

- ✅ Meta descriptions

- ✅ Schema.org markup

- ✅ Sitemap (17 files)

- ✅ robots.txt---# Install dependencies

- ✅ Open Graph tags

npm install

### Performance Optimizations

- ⚡ Code splitting## 🛠️ Tech Stack# or

- ⚡ Image optimization (WebP)

- ⚡ CSS optimizationyarn install

- ⚡ JS minification

- ⚡ Preconnect hints### Frontend# or

- ⚡ Resource preloading

- ⚡ Service worker- **React 18.3** - UI librarybun install



---- **TypeScript 5.8** - Type safety```



## 📱 Browser Support- **Vite 5.4** - Build tool (lightning fast ⚡)



| Browser | Minimum Version |- **Tailwind CSS 3.4** - Utility-first CSS### Development

|---------|----------------|

| Chrome | 90+ ✅ |- **Framer Motion 11** - Smooth animations

| Firefox | 88+ ✅ |

| Safari | 14+ ✅ |- **Radix UI** - Accessible components```bash

| Edge | 90+ ✅ |

- **Lucide React** - Beautiful icons# Start development server

**Mobile**: iOS 14+, Android 5.0+

npm run dev

---

### State Management & Forms# or

## 🤝 Contributing

- **React Hook Form** - Form managementyarn dev

This is a proprietary project. Please contact the owner before making changes.

- **Zod** - Schema validation# or

### Development Workflow

- **React Router DOM** - Client-side routingbun dev

1. Fork the repository

2. Create feature branch (`git checkout -b feature/name`)

3. Commit changes (`git commit -m 'Add feature'`)

4. Push to branch (`git push origin feature/name`)### Development Tools# Open http://localhost:5173 in your browser

5. Open Pull Request

- **ESLint** - Code linting```

### Code Style

- Follow existing patterns- **PostCSS** - CSS processing

- Use TypeScript

- Test on desktop & mobile- **TypeScript ESLint** - TS linting### Build for Production

- Test in both languages

- **Autoprefixer** - CSS vendor prefixes

---

```bash

## 📝 License

### Deployment & Hosting# Create optimized production build

**Proprietary and Confidential**

- **Cloudflare Pages** - Global CDNnpm run build

© 2024-2025 TAAWIDATY. All Rights Reserved.

- **GitHub Actions** - CI/CD (automated)

Unauthorized copying, distribution, or modification is strictly prohibited.

- **Cloudflare Analytics** - Privacy-first analytics# Preview production build locally

---

npm run preview

## 👨‍💻 Developer

---```

<div align="center">



### **BENTALBA ZAKARIA**

## 📊 Database### Linting

[![GitHub](https://img.shields.io/badge/GitHub-@bentalba-181717?style=for-the-badge&logo=github)](https://github.com/bentalba)



**Full-Stack Developer • React Specialist • Morocco 🇲🇦**

### Medication Data```bash

*Designed & Developed with ❤️ in Morocco*

- **Total Medications**: 8,111# Run ESLint
</div>


- **CNSS Database**: Comprehensive coverage```
- **Moroccan Government** - Official medication data


- **Open Source Community** - Amazing tools

---


## 📞 Support
- DCI (International Nonproprietary Name)```


[https://taawidaty.ma](https://taawidaty.ma)- PPV (Pharmacy Public Price)dawa-calcul-plus/

- **Support**: support@taawidaty.ma



Found a bug? Open an issue with:

- Description- Presentation & dosage│   └── robots.txt                  # SEO configuration


- Screenshots

- Browser & device info---│   ├── components/



---│   │   ├── ui/                     # shadcn/ui components



## 📊 Stats## 🎨 Screenshots│   │   ├── AdBanner.tsx           # Advertisement component



<div align="center">│   │   ├── LanguageToggle.tsx     # Language switcher



![GitHub repo size](https://img.shields.io/github/repo-size/bentalba/final-taawidaty?style=flat-square)<div align="center">│   │   ├── ResultCard.tsx         # Medication result display

![GitHub code size](https://img.shields.io/github/languages/code-size/bentalba/final-taawidaty?style=flat-square)

│   │   └── SearchInput.tsx        # Search component

**⭐ Star this repository if you find it helpful!**

### 🏠 Landing Page│   ├── contexts/

</div>

![Landing Page](docs/screenshots/landing-page.png)│   │   └── LanguageContext.tsx    # Global language state

---

*Modern hero section with gradient animations*│   ├── data/

## 🗺️ Roadmap

│   │   ├── medications-cnops.json # CNOPS medication database (5,709 entries)

### ✅ Completed

- [x] 8,111 medication database### 🔍 Smart Search│   │   ├── medications-cnss.json  # CNSS medication database (5,709 entries)

- [x] Bilingual interface (FR/AR)

- [x] CNOPS/CNSS calculators![Search Interface](docs/screenshots/search.png)│   │   └── medicationsLoader.ts   # Async data loader

- [x] Dark mode

- [x] Mobile optimization*Instant autocomplete with fuzzy matching*│   ├── hooks/

- [x] SEO optimization (100/100)

- [x] Performance optimization (95+)│   │   ├── use-mobile.tsx         # Mobile detection hook



### 🔮 Future### 💰 Reimbursement Calculator│   │   ├── use-toast.ts           # Toast notifications

- [ ] User accounts

- [ ] Medication comparison![Calculator](docs/screenshots/calculator.png)│   │   └── useLanguage.ts         # Language hook export

- [ ] Price alerts

- [ ] Pharmacy locator*Calculate exact reimbursement in real-time*│   ├── lib/

- [ ] Mobile app

- [ ] API for developers│   │   ├── faqData.ts             # FAQ content (FR/AR)



---### 🌙 Dark Mode│   │   ├── translations.ts        # UI translations (FR/AR)



<div align="center">![Dark Mode](docs/screenshots/dark-mode.png)│   │   └── utils.ts               # Utility functions



**Made with ❤️ by Moroccan developers, for Moroccans***Beautiful dark theme for night usage*│   ├── pages/



**تعويضاتي - لأن صحتك تستحق الأفضل**│   │   ├── Index.tsx              # Landing page & calculator



*Because your health deserves the best*### 📱 Mobile Responsive│   │   ├── FaqCnops.tsx           # CNOPS FAQ page



---![Mobile View](docs/screenshots/mobile.png)│   │   ├── FaqCnss.tsx            # CNSS FAQ page



[⬆ Back to Top](#taawidaty--تعويضاتي)*Optimized for all screen sizes*│   │   └── NotFound.tsx           # 404 page



</div>│   ├── App.tsx                     # Root component & routing


</div>│   └── main.tsx                    # Application entry point

├── package.json

---├── vite.config.ts                  # Vite configuration

├── tailwind.config.ts              # Tailwind configuration

## 🚀 Getting Started└── tsconfig.json                   # TypeScript configuration

```

### Prerequisites

- Node.js 18+ or Bun---

- Git

## 📖 Usage

### Installation

### 1. Select Insurance Type

```bash

# Clone the repositoryChoose between **CNOPS** or **CNSS** on the landing page.

git clone https://github.com/bentalba/final-taawidaty.git

cd final-taawidaty### 2. Search for Medication



# Install dependenciesType the medication name in the search field. Results appear instantly as you type.

npm install

# or### 3. View Reimbursement Details

bun install

Click on any medication to see:

# Start development server- **Code**: Official medication code

npm run dev- **Prix Public**: Retail price in MAD

# or- **Taux**: Reimbursement percentage

bun dev- **Montant Remboursé**: Amount covered by insurance

```- **Reste à Charge**: Amount you pay



Visit `http://localhost:5173` to see the app in action! 🎉### 4. Access FAQ



### Build for ProductionClick "FAQ" buttons to learn more about:

- Reimbursement procedures

```bash- Required documents

# Build the application- Coverage limits

npm run build- Processing times

- And much more...

# Preview production build

npm run preview---

```

## 🌐 Bilingual Support

---

### Language Toggle

## 📁 Project Structure

Click the language toggle button (🇫🇷/🇦🇪) in the top-right corner to switch between French and Arabic.

```

taawidaty/### RTL Support

├── 📂 src/

│   ├── 📂 components/         # React componentsWhen Arabic is selected:

│   │   ├── 📂 ui/            # Reusable UI components (Radix UI)- **Direction**: Automatic RTL (Right-to-Left) layout

│   │   ├── AdBanner.tsx      # Ad component- **Typography**: Optimized Arabic fonts

│   │   ├── LanguageToggle.tsx- **UI Elements**: Mirrored for natural reading flow

│   │   ├── ResultCard.tsx    # Medication result display

│   │   └── SearchInput.tsx   # Smart search component### Translation Coverage

│   ├── 📂 data/              # Medication databases

│   │   ├── medications-cnops.json  # CNOPS database (8,111 items)- ✅ All UI elements

│   │   ├── medications-cnss.json   # CNSS database- ✅ Button labels

│   │   └── medicationsLoader.ts    # Data loading logic- ✅ Form placeholders

│   ├── 📂 hooks/             # Custom React hooks- ✅ Error messages

│   │   ├── useLanguage.ts    # Language switching- ✅ FAQ content (30 questions total)

│   │   ├── use-mobile.tsx    # Mobile detection- ✅ Informational text

│   │   └── use-toast.ts      # Toast notifications

│   ├── 📂 lib/               # Utility functions---

│   │   ├── faqData.ts        # FAQ content

│   │   ├── translations.ts   # Bilingual content## ❓ FAQ System

│   │   └── utils.ts          # Helper functions

│   ├── 📂 pages/             # Page components### CNOPS FAQ (15 Questions)

│   │   ├── Index.tsx         # Landing page

│   │   ├── PriceChecker.tsx  # Main calculatorCovers topics including:

│   │   ├── Blog.tsx          # Blog/guides- Online access and registration

│   │   └── [...legal pages]- Document requirements

│   ├── App.tsx               # Main app component- Coverage limits and rates

│   ├── main.tsx              # Entry point- Chronic disease management

│   └── index.css             # Global styles- Reimbursement timelines

├── 📂 public/                # Static assets- And more...

│   ├── 📂 logos/             # Brand assets

│   ├── sitemap-index.xml     # Master sitemap### CNSS FAQ (15 Questions)

│   ├── sitemap.xml - sitemap-17.xml  # URL sitemaps

│   └── robots.txt            # SEO directivesCovers topics including:

├── 📂 docs/                  # Documentation- Beneficiary eligibility

│   ├── DEPLOYMENT.md         # Deployment guide- Third-party payment system

│   ├── SEO_GUIDE.md          # SEO best practices- Reimbursement procedures

│   └── [...other docs]- Coverage calculations

├── 📄 generate-sitemap-index.py  # Sitemap generator- Required documentation

├── 📄 package.json           # Dependencies- And more...

├── 📄 vite.config.ts         # Vite configuration

├── 📄 tailwind.config.ts     # Tailwind configuration**Access**: Available from landing page via FAQ Quick Access Banner and Help Cards

└── 📄 tsconfig.json          # TypeScript config

```---



---## 🤝 Contributing



## 🌍 Bilingual SupportWe welcome contributions! Here's how you can help:



### Language Features### Reporting Issues



| Feature | French | Arabic |Found a bug or have a suggestion? [Open an issue](https://github.com/salma1-create/dawa-calcul-plus/issues)

|---------|--------|--------|

| **Interface** | ✅ Full | ✅ Full |### Pull Requests

| **Medication Names** | ✅ | ✅ |

| **Search** | ✅ Autocomplete | ✅ Autocomplete |1. Fork the repository

| **Text Direction** | LTR | RTL ✅ |2. Create your feature branch: `git checkout -b feature/amazing-feature`

| **Font Support** | Latin | Arabic (Noto Sans Arabic) |3. Commit your changes: `git commit -m 'Add some amazing feature'`

| **SEO** | ✅ Optimized | ✅ Optimized |4. Push to the branch: `git push origin feature/amazing-feature`

5. Open a Pull Request

### Switching Languages

Click the language toggle button (🇫🇷/🇲🇦) in the top navigation to switch between French and Arabic instantly.### Development Guidelines



---- Follow TypeScript best practices

- Maintain consistent code style (use ESLint)

## 🔍 SEO & Performance- Write meaningful commit messages

- Test thoroughly before submitting

### SEO Optimization- Update documentation as needed

- ✅ **100/100 Lighthouse SEO score**

- ✅ **8,124 URLs indexed** (100% coverage)---

- ✅ **Proper H1 tags** on all pages

- ✅ **Meta descriptions** optimized## 📊 Medication Data

- ✅ **Schema.org markup** implemented

- ✅ **Sitemap** (17 files, 500 URLs each)### Data Sources

- ✅ **robots.txt** configured

- ✅ **Canonical URLs** set- **CNOPS Database**: 5,709 medications with official codes and rates

- ✅ **Open Graph** tags for social sharing- **CNSS Database**: 5,709 medications with official codes and rates



### Performance Optimizations### Data Format

- ⚡ **Code splitting** (React.lazy)

- ⚡ **Image optimization** (WebP + lazy loading)```json

- ⚡ **CSS optimization** (PurgeCSS + minification){

- ⚡ **JS minification** (esbuild)  "code": "3400893580103",

- ⚡ **Preconnect hints** for third-party domains  "name": "DOLIPRANE 1000 MG CPR BT/8",

- ⚡ **Resource preloading** for critical assets  "publicPrice": 12.50,

- ⚡ **Service worker** for offline functionality  "reimbursementRate": 70

}

---```



## 📱 Browser Support### Updating Data



| Browser | Minimum Version |To update medication databases:

|---------|----------------|

| Chrome | 90+ ✅ |```bash

| Firefox | 88+ ✅ |# Process new medication data

| Safari | 14+ ✅ |python process_medications_cnops.py

| Edge | 90+ ✅ |python process_medications_cnss.py

| Opera | 76+ ✅ |

| Samsung Internet | 14+ ✅ |# Validate JSON structure

node test_medications.mjs

**Mobile Support**: iOS 14+, Android 5.0+```



------



## 🤝 Contributing## 📄 License



We welcome contributions! However, this is a proprietary project. Please contact the owner before making significant changes.This project is licensed under the **MIT License** with the following conditions:



### Development Workflow### ✅ What you CAN do:

- Use the code freely for personal or commercial projects

1. Fork the repository- Modify and distribute the code

2. Create a feature branch (`git checkout -b feature/amazing-feature`)- Use it as a learning resource

3. Commit your changes (`git commit -m 'Add amazing feature'`)

4. Push to the branch (`git push origin feature/amazing-feature`)### ⛔ What you CANNOT do:

5. Open a Pull Request- **Use the name "TAAWIDATY" (تعويضاتي)** - The brand name is protected and proprietary

- Remove or modify author attribution

### Code Style

- Follow existing code patterns### Attribution Required:

- Use TypeScript for type safetyAll copies or substantial portions of the software must include:

- Write meaningful commit messages- **Original Author**: BENTALBA ZAKARIA

- Test on both desktop and mobile- Link to this repository

- Test in both French and Arabic

---

---

## 👨‍💻 Author

## 📝 License

**BENTALBA ZAKARIA**

This project is proprietary and confidential. All rights reserved by **BENTALBA ZAKARIA**.

- Design & Development: Complete UI/UX and full-stack implementation

**© 2024-2025 TAAWIDATY. All Rights Reserved.**- GitHub: [@salma1-create](https://github.com/salma1-create)

- Year: 2025

Unauthorized copying, distribution, or modification of this software is strictly prohibited without explicit written permission from the owner.

*This project was conceived, designed, and built from scratch by BENTALBA ZAKARIA to serve the Moroccan community.*

---

---

## 👨‍💻 Developer

## 🙏 Acknowledgments

<div align="center">

- **CNOPS**: For insurance framework reference

### **BENTALBA ZAKARIA**- **CNSS**: For insurance framework reference

- **shadcn/ui**: For beautiful component library

[![GitHub](https://img.shields.io/badge/GitHub-@bentalba-181717?style=for-the-badge&logo=github)](https://github.com/bentalba)- **Radix UI**: For accessible primitives

[![Portfolio](https://img.shields.io/badge/Portfolio-bentalba.com-0077be?style=for-the-badge&logo=google-chrome&logoColor=white)](https://bentalba.com)- **Lucide**: For icon system



**Full-Stack Developer • React Specialist • Morocco 🇲🇦**---



*Designed & Developed with ❤️ in Morocco*## 📞 Contact



</div>**Author & Maintainer**: BENTALBA ZAKARIA



---**Repository**: [dawa-calcul-plus](https://github.com/salma1-create/dawa-calcul-plus)



## 🙏 Acknowledgments---



- **Moroccan Government** - Official medication pricing data<div align="center">

- **CNOPS** - Insurance reimbursement guidelines  

- **CNSS** - Social security information  ### ⭐ Star us on GitHub — it motivates us a lot!

- **Open Source Community** - Amazing tools and libraries  

- **Users** - For valuable feedback and support  Made with ❤️ for the Moroccan community by **BENTALBA ZAKARIA**

  

---  **TAAWIDATY** © 2025 BENTALBA ZAKARIA

  

## 📞 Support & Contact</div>


### 🌐 Website
**Live**: [https://taawidaty.ma](https://taawidaty.ma)

### 📧 Email
**Support**: support@taawidaty.ma  
**Business**: contact@taawidaty.ma

### 🐛 Bug Reports
Found a bug? Please open an issue on GitHub with:
- Description of the bug
- Steps to reproduce
- Expected vs actual behavior
- Screenshots (if applicable)
- Browser and device information

### 💡 Feature Requests
Have an idea? We'd love to hear it! Open an issue labeled "enhancement".

---

## 📊 Project Stats

<div align="center">

![GitHub repo size](https://img.shields.io/github/repo-size/bentalba/final-taawidaty?style=flat-square)
![GitHub code size](https://img.shields.io/github/languages/code-size/bentalba/final-taawidaty?style=flat-square)
![Lines of code](https://img.shields.io/tokei/lines/github/bentalba/final-taawidaty?style=flat-square)

**⭐ Star this repository if you find it helpful!**

</div>

---

## 🗺️ Roadmap

### ✅ Completed
- [x] 8,111 medication database
- [x] Bilingual interface (FR/AR)
- [x] CNOPS/CNSS calculators
- [x] Dark mode
- [x] Mobile optimization
- [x] SEO optimization (100/100)
- [x] Performance optimization (95+)

### 🚧 In Progress
- [ ] User accounts & saved medications
- [ ] Medication comparison tool
- [ ] Price alerts & notifications
- [ ] Pharmacy locator

### 🔮 Future Plans
- [ ] Mobile app (iOS/Android)
- [ ] API for developers
- [ ] Integration with pharmacy systems
- [ ] Health insurance integration
- [ ] AI-powered medication recommendations

---

<div align="center">

### ⭐ Star History

[![Star History Chart](https://api.star-history.com/svg?repos=bentalba/final-taawidaty&type=Date)](https://star-history.com/#bentalba/final-taawidaty&Date)

---

**Made with ❤️ by Moroccan developers, for Moroccans**

**تعويضاتي - لأن صحتك تستحق الأفضل**

[⬆ Back to Top](#-taawidaty--تعويضاتي)

</div>
